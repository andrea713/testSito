    ...
    return (
        <nav className='material w-full fixed top-0 z-20'>
            <div className='relative m-auto flex items-center justify-between max-w-[1400] p-2.5 md:p-2.5'>
                ...
                <Link href={"/"}><LogoStudioAdorante/></Link>

                <div className='md:hidden '>
                    <button
                    onClick={()=> setOpen(!open)} className='text-3xl align-top relative  '
                    >
                        {open ? <IoClose/> : <IoMenu/>}
                    </button>
                ...
                <Link href={"/Contatti"} className='hidden button md:visible'>
                    <IoCall/>
                    <span className='hidden'>Contattaci</span>
